...
module.exports = {
    ...
    aliases: ["ft", "ftn"],
    category: "fortnite",
    description: "Afficher les statistiques d'un utilisateur et le shop actuel",
    ...
    run: async (client, message, args) => {
        ...
        if (args[0].toLowerCase() === "store") {
            ...
            const embed = new RichEmbed()
                .setColor("#9d4dbb")
                .setFooter("Limen by Sckz", message.author.displayAvatarURL)
                .setTimestamp();
            ...
            if (!search.username) {
                return message.channel.send("> Joueur introuvable !")
            }
            ...
            const embed = new RichEmbed()
                ...
                .setURL(search.url)
                .setColor("#9d4dbb")
                .setFooter(`Limen by Sckz`, client.user.displayAvatarURL)
                .setTimestamp()
                .addField("Solo:", stripIndents`**- Wins:** ${solo.wins}
                **- KD:** ${solo.kd}
                **- Kills:** ${solo.kills}
                **- Kills par match:** ${solo.kills_per_match}`, true)
                .addField("Duo:", stripIndents`**- Wins:** ${duo.wins}
                **- KD:** ${duo.kd}
                **- Kills:** ${duo.kills}
                **- Kills par match:** ${duo.kills_per_match}`, true)
                .addField("Squad:", stripIndents`**- Wins:** ${squad.wins}
                **- KD:** ${squad.kd}
                **- Kills:** ${squad.kills}
                **- Kills par match:** ${squad.kills_per_match}`, true)
